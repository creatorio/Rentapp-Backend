...
const defaultCryptoKeyDataFormat = "jwk";
function int8ArrayEqual(array1, array2) {
  if (array1 === array2) {
    return true;
  }
...
function generateSigningKeyPair() {
  return env.crypto.subtle.generateKey(defaultAsymmetricSigningParam, true, [
    "sign",
    "verify",
  ]);
}
function generateEncryptionKeyPair() {
  return env.crypto.subtle.generateKey(defaultAsymmetricEncryptionParam, true, [
    "encrypt",
    "decrypt",
  ]);
}
...
function importKey(key) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    if (key.key_ops) {
      ...
      return env.crypto.subtle.importKey(
...
function sign(plaintext, signingPrivateKey) {
  return __awaiter(this, void 0, void 0, function* () {
    const signature = yield env.crypto.subtle.sign(
...
function verifySignature(plaintext, signature, signingPublicKey) {
  return __awaiter(this, void 0, void 0, function* () {
    return env.crypto.subtle.verify(
      defaultSigningParams,
      signingPublicKey,
      signature,
      plaintext
...
function encrypt(plaintext, encryptionPublicKey) {
  return __awaiter(this, void 0, void 0, function* () {
    const ciphertext = yield env.crypto.subtle.encrypt(
...
function decrypt(ciphertext, encryptionPrivateKey) {
  return __awaiter(this, void 0, void 0, function* () {
    const plaintext = yield env.crypto.subtle.decrypt(
      defaultEncryptParams,
...
var asymmetricCrypto = /*#__PURE__*/ Object.freeze({
...
const encryptionAlgo$1 = "AES-GCM";
const keySize = 256;
const ivLength = 12;
const tagLength = 128;
const defaultSymmetricEncryptionParams = {
  name: encryptionAlgo$1,
  length: keySize,
};
const defaultImportEncryptionParams = {
  name: encryptionAlgo$1,
};
function getDefaultEncryptParams(generateNewNonce) {
  if (generateNewNonce) {
    return {
      name: encryptionAlgo$1,
      iv: generateNonce(),
...
function joinNonceCiphertext(nonce, ciphertext) {
...
function splitNonceCiphertext(data) {
  return new Promise((resolve) => {
    let result;
...
function generateNonce() {
  return env.crypto.getRandomValues(new Uint8Array(ivLength));
}
...
const g = new bn_1(2);
const riSize = 64;
function intArrayToBN(array) {
  return new bn_1(array);
}
function BNToIntArray(bn) {
  return bn.toArrayLike(Uint8Array);
}

const red = bn_1.mont(p);
function deriveKey(sharedSecret) {
  return __awaiter(this, void 0, void 0, function* () {
    const HKDFKey = yield env.crypto.subtle.importKey(
      KDFKeyParams.format,
      sharedSecret,
...
function generateRi() {
  return intArrayToBN(env.crypto.getRandomValues(new Uint8Array(riSize)));
}
function computeZi(ri) {
  const res = g.toRed(red).redPow(ri).fromRed();
  return BNToIntArray(res);
}
function computeXi(ri, ziRight, ziLeft) {
  const modInvM = new bn_1(ziLeft).toRed(red).redInvm();
  const res = modInvM.redMul(new bn_1(ziRight).toRed(red)).redPow(ri);
  return BNToIntArray(res.fromRed());
}
function computeSharedSecret(ri, xi, ziLeft, xiList) {
  const res = new bn_1(ziLeft).toRed(red).redPow(ri.muln(xiList.length));
  const iInit = indexOf(xiList, xi);
  const exponent = new bn_1(xiList.length - 1);
  let xin;
  let ind;
  for (let i = 0; exponent.gtn(0); i++) {
    ind = (iInit + i) % xiList.length;
    xin = new bn_1(xiList[ind]).toRed(red).redPow(exponent);
    res.redIMul(xin);
    exponent.isubn(1);
  }
  return BNToIntArray(res.fromRed());
}
function indexOf(xiList, xi) {
  let indexOfXi = -1;
...
var keyAgreementCrypto = /*#__PURE__*/ Object.freeze({
  deriveKey: deriveKey,
  generateRi: generateRi,
  computeZi: computeZi,
  computeXi: computeXi,
...
function exportKey$1(cryptoKey) {
  return __awaiter(this, void 0, void 0, function* () {
    return env.crypto.subtle.exportKey(defaultCryptoKeyDataFormat, cryptoKey);
  });
}
function toB64(keyData) {
  return btoa(JSON.stringify(keyData));
}
function fromB64(keyDataB64) {
  return JSON.parse(atob(keyDataB64));
}
function importKey$1(cryptoKeyData) {
  return __awaiter(this, void 0, void 0, function* () {
    return env.crypto.subtle.importKey(
      defaultCryptoKeyDataFormat,
      cryptoKeyData,
...
function encrypt$1(plaintext, encryptionKey) {
  return __awaiter(this, void 0, void 0, function* () {
...
function decrypt$1(data, encryptionKey) {
  return __awaiter(this, void 0, void 0, function* () {
    const [nonce, ciphertext] = yield splitNonceCiphertext(data);
    const params = getDefaultEncryptParams(false);
    params.iv = nonce;
    const buffer = yield env.crypto.subtle.decrypt(
      params,
      encryptionKey,
      ciphertext
    );
...
var symmetricCrypto = /*#__PURE__*/ Object.freeze({
...
//# sourceMappingURL=mute-crypto-helper.browser.es2015.esm.js.map
